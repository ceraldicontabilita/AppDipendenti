...
async def processa_cedolino_completo(
    ...
    try:
        ...
        try:
            from backend.app.core.event_bus import bus
        ...
        if cedolino_data.get("cessato") and dipendente_id:
            try:
                ...
                if not gia_cessato:
                    ...
                    from backend.app.services.event_bus import propagate_event as _pe, EventTypes as _et
...
async def processa_tutti_cedolini_pdf(db, pdf_data: str, filename: str) -> Dict[str, Any]:
    ...
    try:
        from backend.app.services.document_ai_extractor import extract_document_data
    ...
    if not cedolini:
        try:
            from backend.app.parsers.payslip_parser_v2 import parse_payslip_pdf
    ...
    for ced in cedolini:
        ...
        if ced_pdf_text and not ced.get("cessato"):
            try:
                from backend.app.parsers.busta_paga_multi_template import detect_cessazione
        ...
        try:
            from backend.app.services.salari_unificati_v2 import processa_cedolino_v2
